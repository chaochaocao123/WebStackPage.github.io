...
import { autoSlug, autoExcerpt } from '@/lib/article-utils';
...
export function ArticleFormClient({ initialData, formAction, draftAction, deleteAction, pushToBaiduAction, baiduQuotaInitial }: Props) {
  ...
  const [title, setTitle] = useState(initialData?.prefillTitle || initialData?.title || '');
  const [slug, setSlug] = useState(initialData?.prefillSlug || initialData?.slug || '');
