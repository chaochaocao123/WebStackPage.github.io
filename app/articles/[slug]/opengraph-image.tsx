...
/** 从 Article.content 提取纯文本前 N 字（用于 OG 描述） */
...
export default async function ArticleOpengraphImage({ params }: { params: { slug: string } }) {
  const item = await prisma.article.findUnique({ where: { slug: params.slug } });
  if (!item) {
    return new ImageResponse(<DefaultOG title="文章不存在" />, size);
  }

  const desc = item.excerpt || extractText(item.content || '', 80);
